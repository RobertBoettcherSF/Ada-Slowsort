--  Standalone test suite for Slowsort (main program).
--  CRITICAL: keep n tiny — Slowsort is superpolynomial.

pragma Ada_2022;

with Ada.Text_IO; use Ada.Text_IO;
with Slowsort; use Slowsort;
...
begin
   ...
   ---------------------------------------------------------------------
   Section ("6. One cautious n=20 case");
   ---------------------------------------------------------------------
   --  Data order does not help Slowsort, but n=20 is still tractable in Ada.
   Expect_Sorted (Random_Array (20, -5, 5), "random n=20 range -5..5");
   ...
   --  Do NOT call Sort on Max_N (=24) — that would hang. Only verify that
   --  a modest legal length is accepted and Max_N+1 is rejected (above).
   declare
   ...
   if Fail_Count /= 0 then
      raise Program_Error with "Slowsort tests failed";
   end if;
end Tests;
